...
def write_summary_total2(args, config_str, stats):
    f = open("results/{}/{}_{}_total.txt"
             .format(args.dataset, args.embedder, args.fn), "a")
    f.write("--------------------------------------------------------------------------------- \n")
    f.write(config_str)
    f.write("\n")
    # 写入标题行
    f.write("ROC (mean)\tROC (std)\tAP (mean)\tAP (std)\tF1 (mean)\tF1 (std)\tAcc (mean)\tAcc (std)\n")
