    ...
    def train(self):        
        ...
        for epoch in range(1, self.args.epochs + 1):
            ...
            for bc, samples in enumerate(self.train_loader):
                ...
                outputs, KL_Loss, cont_loss, preserve_rate, vq_loss = self.model([samples[0].to(self.device), samples[1].to(self.device), masks[0].to(self.device), masks[1].to(self.device)], bottleneck = True)
                loss += loss_function_BCE(outputs, samples[2].reshape(-1, 1).to(self.device).float()).mean()
                loss += self.args.beta * KL_Loss
                loss += self.args.beta * cont_loss

                loss += self.args.beta * vq_loss
